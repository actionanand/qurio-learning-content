...
## Client-facing display metadata

`manifest.json` includes lightweight localized display metadata generated by Qurio Content Studio. This lets the learner client render readable titles for linked exam-plan study material and practice sets without fetching every source file first.

For notes/syllabi the manifest can include localized `title`; notes also include lightweight `difficulty` and `estimatedMinutes`. For quizzes it can include localized `title` and `setLabel`, plus `difficulty`, `timeLimitSeconds`, `passingPercentage` and `questionCount`.

The Studio refreshes these fields automatically whenever normal content or a translation is saved. Use **Dashboard → Sync client metadata** or **Validate → Rebuild & sync metadata** after manual filesystem edits.

Exam calendar files should continue referencing stable IDs such as `sci-05-photosynthesis`; do not duplicate display titles into calendar JSON.
